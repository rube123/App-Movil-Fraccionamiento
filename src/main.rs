...
struct GoogleLoginRequest {
    correo: String, // solo necesitamos el correo
}
...
async fn login_google(
    ...
    // 1) Buscar usuario+persona por correo_login
    ...
    let usuario_row = match usuario_row {
        Ok(Some(row)) => row,
        Ok(None) => {
            // ⛔ correo de Google NO está registrado en tabla usuario
            return HttpResponse::Unauthorized()
                .body("Correo no registrado en el fraccionamiento");
        }
        Err(e) => {
            return HttpResponse::InternalServerError()
                .body(format!("Error buscando usuario: {e}"));
        }
    };
    ...
    let roles = match roles_rows {
        ...
        Err(e) => {
            return HttpResponse::InternalServerError()
                .body(format!("Error obteniendo roles: {e}"));
        }
    };
    ...
    // 4) Respuesta igual que /login normal
...
async fn get_fcm_access_token() -> Result<String, Box<dyn std::error::Error>> {
    println!("🔐 get_fcm_access_token: leyendo /etc/secrets/fcm_service_account.json");

    // Lee el service account de fcm_service_account.json
    let key = read_service_account_key("/etc/secrets/fcm_service_account.json").await?;
    ...
    println!("✅ get_fcm_access_token: token obtenido (longitud = {})", token_str.len());
...
async fn send_fcm_push(
    ...
    println!("📨 send_fcm_push llamado");
    println!("👉 Destinatarios (id_persona): {:?}", destinatarios);
    ...
    println!("👉 Tokens encontrados: {:?}", tokens);
    ...
    let project_id = env::var("FCM_PROJECT_ID")?; // Debe ser el mismo que `project_id` del JSON
    let url = format!(
    ...
    println!("👉 URL FCM: {}", url);
    println!(
        "👉 Bearer (primeros 20 chars): {}",
        bearer.chars().take(20).collect::<String>()
    );
    ...
    for t in tokens {
        ...
        println!("👉 Enviando FCM a token: {}", t);
...
async fn crear_aviso(pool: web::Data<PgPool>, body: web::Json<AvisoInput>) -> impl Responder {
    println!("📣 /avisos (crear_aviso) llamado con body = {:?}", &*body);

    // Validar que el emisor tiene rol admin/mesa
    match emisor_puede_enviar(pool.get_ref(), body.id_usuario_emisor).await {
        Ok(true) => {}
        Ok(false) => {
            println!("❌ crear_aviso: emisor {} NO tiene permiso", body.id_usuario_emisor);
            return HttpResponse::Forbidden().body("No tienes permiso");
        }
        Err(e) => {
            println!("❌ crear_aviso: error verificando permisos: {e}");
    ...
    let id_aviso = match id_aviso_res {
        Ok(v) => {
            println!("✅ crear_aviso: aviso insertado con id_aviso = {}", v);
            v
        }
        Err(e) => {
            println!("❌ crear_aviso: error al insertar aviso: {e}");
            return HttpResponse::InternalServerError().body(e.to_string());
        }
    };

    // Si NO es a todos, insertamos en aviso_destinatario
    if !body.a_todos {
        let destinatarios = body.destinatarios.clone().unwrap_or_default();
        println!("👉 crear_aviso: destinatarios específicos = {:?}", destinatarios);
        ...
        for id_persona in &destinatarios {
            ...
            if let Err(e) = res {
                println!(
                    "⚠️ crear_aviso: error insertando aviso_destinatario (id_persona={}): {e}",
                    id_persona
                );
            }
        }
    ...
    // Construimos lista final de personas que deben recibir push
    let recipients: Vec<i32> = if body.a_todos {
        ...
        let rows = sqlx::query_as::<_, PersonaIdRow>("SELECT id_persona FROM persona")
            .fetch_all(pool.get_ref())
            .await
            .unwrap_or_default();

        let ids: Vec<i32> = rows.into_iter().map(|r| r.id_persona).collect();
        println!("👉 crear_aviso: a_todos = TRUE, recipients = {:?}", ids);
        ids
    } else {
        let ids = body.destinatarios.clone().unwrap_or_default();
        println!("👉 crear_aviso: a_todos = FALSE, recipients = {:?}", ids);
        ids
    };

    // Enviar push (los errores no rompen la respuesta HTTP)
    let push_res = send_fcm_push(pool.get_ref(), &body.titulo, &body.mensaje, &recipients).await;
    if let Err(e) = push_res {
        println!("⚠️ Error al enviar FCM: {e}");
    }
